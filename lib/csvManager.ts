...
class CSVManager {
  ...
  readCSV(filename: string): Record<string, any>[] {
    try {
      const filePath = path.join(this.dataDir, filename);
      if (!fs.existsSync(filePath)) {
  ...
  writeToCSV(filename: string, data: Record<string, any>): boolean {
    try {
      const filePath = path.join(this.dataDir, filename);
  ...
  updateCSV(filename: string, id: string, updateData: Record<string, any>): boolean {
    try {
  ...
  deleteFromCSV(filename: string, id: string): boolean {
    try {
      const data = this.readCSV(filename);
  ...
  private rewriteCSV(filename: string, data: Record<string, any>[]): void {
    const filePath = path.join(this.dataDir, filename);
    const headers = this.getHeaders(filename);
  ...
  private getHeaders(filename: string): string[] {
  ...
  findById(filename: string, id: string): Record<string, any> | null {
    const data = this.readCSV(filename);
    return data.find(row => row.id === id) || null;
  }

  findByField(filename: string, field: string, value: string): Record<string, any>[] {
    const data = this.readCSV(filename);
    return data.filter(row => row[field] === value);
  }

  findOne(filename: string, criteria: Record<string, any>): Record<string, any> | null {
    const data = this.readCSV(filename);
